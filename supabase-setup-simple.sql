...
CREATE OR REPLACE FUNCTION check_user_generation_limit(user_uuid UUID, model_type ai_model)
RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
    SELECT json_build_object(
        ...
        'success_rate', CASE 
            WHEN up.total_generations > 0 THEN 
                ROUND((up.successful_generations::DECIMAL / up.total_generations) * 100, 2)
            ELSE 0
        END,
        'monthly_usage', (
            SELECT COUNT(*) 
...
-- 1. This script creates the basic database structure without RLS policies
-- 2. To create your first super admin user:
...
-- 3. After creating your first user, you can enable RLS policies in Supabase dashboard if needed
-- 4. The application handles authentication and authorization through JWT tokens
